...
async function updateTaskStatus(taskId, newStatus) {
    ...
    try {
        ...
        const response = await fetch(`http://localhost:3000/api/tasks/${taskId}/next-status`, {
            method: 'PATCH',
            headers: {
                'Content-Type': 'application/json'
            },
...
async function deleteTask(taskId) {
    ...
    try {
        ...
        alert('❌ Error: Could not delete task. Please try again.');
    } finally {
...
function createStatusButtons(taskId, currentStatus) {
    ...
    // เงื่อนไขสำหรับปุ่ม "← To Do"
    // จะแสดงเมื่อสถานะปัจจุบันไม่ใช่ TODO (คืออยู่ใน IN_PROGRESS หรือ DONE)
